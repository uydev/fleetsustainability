...
# Always operate from the repository root so writes go to a writable path
cd "$REPO_ROOT"

# --- EC2 compatibility shims (do NOT remove) ---
# Detect API base (8081 for local Docker Compose; 8080 for EC2 container)
detect_api_base() {
    if [ -n "${API_BASE_URL:-}" ]; then echo "$API_BASE_URL"; return; fi
    if command -v curl >/dev/null 2>&1; then
        curl -s -m 2 http://localhost:8081/health >/dev/null 2>&1 && { echo "http://localhost:8081"; return; }
        curl -s -m 2 http://localhost:8080/api/health >/dev/null 2>&1 && { echo "http://localhost:8080"; return; }
        curl -s -m 2 http://localhost:8080/health >/dev/null 2>&1 && { echo "http://localhost:8080"; return; }
    fi
...
__REAL_DATE_BIN__="$(command -v date)"
date() {
    if [ "${1:-}" = "-u" ] && [ "${2:-}" = "-r" ] && [ -n "${3:-}" ]; then
        "${__REAL_DATE_BIN__}" -u -d "@${3}" "${@:4}" 2>/dev/null || "${__REAL_DATE_BIN__}" -u -r "${3}" "${@:4}"
    else
        "${__REAL_DATE_BIN__}" "$@"
    fi
}

# Docker often requires sudo on EC2
docker() { command docker "$@" 2>/dev/null || sudo docker "$@"; }
docker-compose() { command docker-compose "$@" 2>/dev/null || sudo docker-compose "$@"; }

# Ensure OSRM artifacts go under HOME (writable)
export OSRM_DIR="${OSRM_DIR:-$HOME/build/osrm}"
mkdir -p "$OSRM_DIR" 2>/dev/null || true
