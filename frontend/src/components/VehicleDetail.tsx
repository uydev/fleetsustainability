...
const VehicleDetail: React.FC<Props> = ({ vehicles, timeRange, selectedVehicleId }) => {
  ...
  const exportPdf = () => {
    ...
    doc.text(`Vehicle ${vehicleId} Telemetry Report`, pageWidth / 2, yPosition, { align: 'center' });
    yPosition += 20;

    // Vehicle info
    doc.setFontSize(12);
    doc.setFont('helvetica', 'normal');
    doc.text(`Vehicle Type: ${selectedType}`, 20, yPosition);
    yPosition += 10;
    doc.text(`Report Generated: ${new Date().toLocaleString()}`, 20, yPosition);
    yPosition += 10;
    doc.text(`Total Records: ${data?.length || 0}`, 20, yPosition);
    yPosition += 20;
    ...
    if (data && data.length > 0) {
      ...
      const avgFuel = selectedType === 'EV' 
        ? data.reduce((sum, d) => sum + (d.battery_level || 0), 0) / data.length
        : data.reduce((sum, d) => sum + (d.fuel_level || 0), 0) / data.length;
      ...
      doc.setFont('helvetica', 'normal');
      doc.text(`Average Speed: ${avgSpeed.toFixed(2)} km/h`, 20, yPosition);
      yPosition += 8;
      doc.text(`Max Speed: ${maxSpeed.toFixed(2)} km/h`, 20, yPosition);
      yPosition += 8;
      doc.text(`Min Speed: ${minSpeed.toFixed(2)} km/h`, 20, yPosition);
      yPosition += 8;
      doc.text(`Total Emissions: ${totalEmissions.toFixed(2)} g`, 20, yPosition);
      yPosition += 8;
      doc.text(`Average ${selectedType === 'EV' ? 'Battery' : 'Fuel'} Level: ${avgFuel.toFixed(2)}%`, 20, yPosition);
      yPosition += 20;
    }
    ...
    if (data && data.length > 0) {
      doc.setFont('helvetica', 'bold');
      doc.text('Telemetry Data:', 20, yPosition);
      yPosition += 10;

      // Table headers
      const headers = ['Timestamp', 'Speed (km/h)', selectedType === 'EV' ? 'Battery (%)' : 'Fuel (%)', 'Emissions (g)'];
      ...
      // Table data (limit to first 20 records to fit on page)
      ...
      limitedData.forEach((record) => {
        ...
        const speed = record.speed.toFixed(1);
        const level = selectedType === 'EV' ? (record.battery_level || 0).toFixed(1) : (record.fuel_level || 0).toFixed(1);
        const emissions = record.emissions.toFixed(2);

        doc.text(timestamp, xPosition, yPosition);
        xPosition += colWidths[0];
        doc.text(speed, xPosition, yPosition);
        xPosition += colWidths[1];
        doc.text(level, xPosition, yPosition);
        xPosition += colWidths[2];
        doc.text(emissions, xPosition, yPosition);
        yPosition += 6;
      });
